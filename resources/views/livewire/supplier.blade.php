<x-fondo-card>
    ...
    <x-slot name="content">
        <x-notification/>
        <x-a-button id="createSupplier" title="Crear Proveedor" href="{{ route('supp.create') }}" class="p-1 bg-green-800 hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-600">
            Crear
        </x-a-button>
        @if ($suppliers->count()) {{-- Si el array trae registros --}}
            <table class="w-full mt-3 table-auto">
                <thead class="w-full mt-3 table-auto">
                    <tr>
