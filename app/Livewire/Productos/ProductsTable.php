...
final class ProductsTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('avatar', fn($item) => '<a href="' . env('APP_URL') . Storage::url($item->image->url) . '"><img class="w-8 h-8 rounded-full shrink-0 grow-0" src="' .  Storage::url($item->image->url) . '"></a>')
            ->add('created_at');
    }
        ...
        return [
            Button::add('add')
                ->render(function ($pro) {
                    return Blade::render(<<<HTML
                        @can('devi.create')
                            <x-a-button id="addDevice" title="Agregar Dispositivo" wire:click="dispatch('openModal',{component: 'DeviSalvar', arguments: {id: {{ $pro->id }}, idDispo: null }})"  class="mr-2 p-sm-button bg-lime-800 hover:bg-lime-600 focus:outline-none focus:ring-2 focus:ring-violet-600">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
